...
tools directly. In particular, all access to Jira, Bitbucket, and GitLab
MUST go through the corresponding skill (`jira-fetch`, `bitbucket-fetch`,
`gitlab-fetch`) — do not call the `jira_*`, `bitbucket_*`, or `gitlab_*`
MCP tools directly. The skills carry the conventions and guardrails for
those services.
