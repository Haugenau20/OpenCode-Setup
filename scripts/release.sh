#!/usr/bin/env bash
# scripts/release.sh — build, tag, and (optionally) push the two release
...
# ─────────────────────────────────────────────────────────────────────────────
# Registry / image name — the opencode image's full path; the squid image is
# the same with a `-squid` suffix. Resolved in this order (first non-empty
# wins):
...
BUILD_OPENCODE=1 BUILD_SQUID=1
