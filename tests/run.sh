#!/usr/bin/env bash
#
# Run the OpenCode-Setup test suite. Uses a system `bats` if present;
# otherwise fetches bats-core into tests/.bats (gitignored) on first run.
#
...
if [ "$#" -gt 0 ]; then
  # Resolve bare names (e.g. "git-guard.bats") against the tests dir so the
  # suite can be targeted from any working directory; leave explicit paths
  # untouched.
  TARGETS=()
  for t in "$@"; do
    case "$t" in
